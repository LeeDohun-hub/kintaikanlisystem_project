...
| 書く | 書かない（参照先） |
|------|---------------------|
| API の**メソッド/パス/権限/エラー/入力**、処理シーケンス、テスト観点、業務ルールの**実装可能な記述** | ビジネス目的・スコープの議論 → **要件定義書**（または `README_jp.md` 付録） |
| | 技術スタックと画面一覧の**地図** → **`基本設計書.md`** |
| | 会議用の最短俯瞰・図のみ → **`system-design-overview_jp.md`** / `_kr` 相当 |
