...
**文書目的**: 代表・社内レビュー向けに、**構成・認証の概念・役割**を **1ファイル・短時間**で俯瞰できるようにします。  
**守備範囲**: 詳細な API 定義・テーブル列定義は **書かない**（`基本設計書.md` / `詳細設計書.md` / `データ_設計.md` へ誘導）。本書の「APIグループ」「データ概要」は会議用の **目次レベル**に留める。  
**更新**: コード・要件が変わった場合は、本ドキュメントも併せて更新することを推奨します。  
**ダイアグラム**: [Mermaid](https://mermaid.js.org/) 記法です（GitHub/Notion/VS Codeプレビュー等でレンダリング可能）。  
**文書間の役割**: `docs/설계문서_역할_구분.md`
...
| パス | 説明 |
|------|------|
| `docs/설계문서_역할_구분.md` | 要件定義・システム設計・基本・詳細の **役割分担（重複防止）** |
| `docs/基本設計書.md` | 方式・画面一覧・API 概要・ER 概要 |
