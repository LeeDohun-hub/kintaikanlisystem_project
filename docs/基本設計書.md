...
### 1.1 本文書の守備範囲（他文書との重複防止）

| 書く | 書かない（参照先） |
|------|---------------------|
| 方式（セッション、Interceptor 方針）、画面ルート一覧、API の**カテゴリ＋代表パス**、ER **概要**、非機能の**方針** | 業務ルールの検証可能な細目、HTTP ごとの Body 例 → **`詳細設計書.md`**、`インタフェース_設計.md` |
| | 代表向けの短い全体図のみ → **`system-design-overview_jp.md`**（本書と図を二重管理しない） |
| | 列定義・DDL の正本 → **`データ_設計.md`**、Flyway |

**4 種類の文書の役割一覧:** `docs/설계문서_역할_구분.md`
